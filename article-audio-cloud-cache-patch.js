/* article-audio-cloud-cache-patch.js v20260710-3.1
   MP3 + JSON metadata on GitHub. Adds forceRegen().
*/
...
(function () {
  ...
  async function githubPutFile(path, contentB64, message) {
    ...
    var token = localStorage.getItem(KEY_GH_TOKEN);
    if (!token) throw new Error('need GitHub Token');
  ...
  async function coreFresh(text, id) {
    ...
    if (!blobs.length) throw new Error('all segments failed');
  ...
  async function play(text, forcedId) {
    ...
    var fresh = await coreFresh(text, id);
    return { mode: 'fresh', url: fresh.url, id: id, meta: fresh.meta };
  }

  async function forceRegen(text, forcedId) {
  ...
  log('ready v3.1 (mp3 + json metadata + forceRegen)');
