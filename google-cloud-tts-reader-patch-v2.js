...
/* google-cloud-tts-reader-patch-v2.js v20260707-2
   Fix Google Cloud TTS 5000-byte limit by byte-safe chunking.
   Google Cloud Text-to-Speech text:synthesize has a 5,000 bytes input limit per request.
   This version splits article text by UTF-8 bytes, not just characters/sentences.
   Install:
   <script src="./google-cloud-tts-reader-patch-v2.js?v=20260707-2"></script>
*/
(function(){
  'use strict';
  const KEY='notebook_google_cloud_tts_key_v1';
  const SETTINGS='notebook_google_cloud_tts_settings_v1';
  const MAX_BYTES=4500; // keep below Google 5000-byte content limit
  let audio=null;
  let chunks=[];
  let chunkIndex=0;
  let playing=false;
  ...
  function getArticleText(){
    // Only read English article blocks, not Study Notes / side menus.
  ...
  function splitByBytes(text,maxBytes=MAX_BYTES){
    ...
    function pushPiece(piece){
      ...
      if(byteLen(piece)<=maxBytes){
        if(byteLen((buf?buf+' ':'')+piece)<=maxBytes){buf+=(buf?' ':'')+piece;}
        else{flush();buf=piece;}
      }else{
        // Sentence itself too long: split by words.
        const words=piece.split(/\s+/);
        for(const w of words){
          if(byteLen((buf?buf+' ':'')+w)<=maxBytes){buf+=(buf?' ':'')+w;}
          else{flush();
            if(byteLen(w)<=maxBytes){buf=w;}
            else{
              // Rare extremely long token: split by characters safely.
              let small='';
              for(const ch of w){
                if(byteLen(small+ch)<=maxBytes)small+=ch;
                else{if(small)out.push(small);small=ch;}
              }
              if(small)buf=small;
            }
          }
  ...
  async function synthesize(text){
    ...
    if(!res.ok)throw new Error('Google Cloud TTS '+res.status+': '+raw.slice(0,260));
  ...
  function playAll(){
    ...
    chunks=splitByBytes(t,MAX_BYTES);
    chunkIndex=0;
    if(!chunks.length)return alert('分段後沒有文字。');
    console.log('Google Cloud TTS chunks:',chunks.map((c,i)=>({i:i+1,bytes:byteLen(c),preview:c.slice(0,80)})));
    playCurrent();
  }
  ...
  function inject(){
    ...
    console.log('Google Cloud TTS reader patch v2 loaded');
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',inject);else inject();
  window.openGoogleCloudTTSKey=setKey;
  window.googleCloudTTSPlayAll=playAll;
  window.googleCloudTTSSplitDebug=()=>splitByBytes(getArticleText(),MAX_BYTES).map((c,i)=>({i:i+1,bytes:byteLen(c),text:c}));
})();
