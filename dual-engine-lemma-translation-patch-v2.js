...
/* dual-engine-lemma-translation-patch-v2.js
   Fixes: original save() could overwrite db.learn created by patch.
   Adds: syncGlobalDB() so localStorage + original global db stay in sync.
*/
(function(){
  'use strict';
  ...
  function syncGlobalDB(d){
    d=norm(d); d.updatedAt=Date.now();
    localStorage.setItem(STORE,JSON.stringify(d));
    // Important: original app uses global lexical variable `db` in save()/cloudUpload().
    // If we only update localStorage, original save() can overwrite learn data.
    try{ db=d; }catch(e){}
    return d;
  }
  function safeSave(){
    try{ if(typeof save==='function') save(); else syncGlobalDB(getDB()); }catch(e){ syncGlobalDB(getDB()); }
  }
  ...
  function ensureLemma(surface){
    const info=lemmatize(surface), key=info.lemma;
    const d=getDB();
    if(!d.learn[key]) d.learn[key]={word:key,lemma:key,pos:'',tw:'（未建字義，點擊後自動查詢）',tip:'',clicks:0,strength:0,captured:false,known:false,dueAt:0,updatedAt:Date.now(),variants:{},lastSurface:info.surface,lastForm:info.form,engine:''};
    const x=d.learn[key];
    x.word=key; x.lemma=key; x.lastSurface=info.surface; x.lastForm=info.form; x.variants=x.variants||{};
    if(info.surface)x.variants[info.surface]=(x.variants[info.surface]||0)+1;
    syncGlobalDB(d);
    return{key,info,x,db:d};
  }
  ...
  async function autoTranslateLemma(lemma){
    let d=getDB(); const x=d.learn[lemma]; if(!x||x.loading)return;
    if(x.tw && !String(x.tw).includes('未建') && !String(x.tw).includes('查詢中') && !String(x.tw).includes('翻譯 API'))return;
    x.loading=true; x.tw='查詢中...'; x.updatedAt=Date.now(); syncGlobalDB(d); showWord(lemma);
    let tw='', engine='';
    ...
    d=getDB();
    if(d.learn[lemma]){
      d.learn[lemma].tw=tw; d.learn[lemma].engine=engine;
      d.learn[lemma].pos=d.learn[lemma].pos||dict.pos||'';
      d.learn[lemma].tip=(engine?('Auto translated by '+engine):'')+(dict.definition?('｜'+dict.definition):'');
      d.learn[lemma].loading=false; d.learn[lemma].updatedAt=Date.now();
      syncGlobalDB(d);
    }
    showWord(lemma); refreshUI(); safeSave();
  }
  function showWord(surfaceOrLemma){
    const key=lemmatize(surfaceOrLemma).lemma; const d=getDB(); const x=d.learn[key]; if(!x||!document.getElementById('dockBody'))return;
