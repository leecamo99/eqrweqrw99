...
/* google-cloud-tts-reader-patch-v4.js v20260707-4
   Google Cloud TTS + approximate dynamic cursor/highlight.
   Why approximate: Cloud TTS REST text:synthesize returns audioContent only, no per-word timestamps.
   This patch estimates current word by audio.currentTime / audio.duration within each chunk.
   Install:
   <script src="./google-cloud-tts-reader-patch-v4.js?v=20260707-4"></script>
*/
(function(){
  'use strict';
  ...
  function clearHighlight(){
    document.querySelectorAll('.gctts-current').forEach(x=>x.classList.remove('gctts-current'));
  }
  ...
  function splitLongLineToSentencesWithWords(line,startWordIndex){
    ...
    for(const piece0 of raw){
      ...
      if(byteLen(piece)<=MAX_SENTENCE_BYTES){
        result.push({text:asSentence(piece),start:cursor,end:cursor+pieceWords.length});
        cursor+=pieceWords.length;
        continue;
      }
      let buf='', localStart=cursor, localCount=0;
      for(const w of piece.split(/\s+/)){
        const hasWord=/[A-Za-z]/.test(w);
        const candidate=(buf?buf+' ':'')+w;
        if(byteLen(candidate)<=MAX_SENTENCE_BYTES){buf=candidate;if(hasWord)localCount++;}
        else{
          if(buf)result.push({text:asSentence(buf),start:localStart,end:localStart+localCount});
          localStart=localStart+localCount; localCount=hasWord?1:0; buf=w;
        }
      }
  ...
  function buildSentencesFromDOM(){
    wrapArticleWords();
    const out=[];
    // Build line text from wrapped DOM and map words by matching order.
    let globalWord=0;
    document.querySelectorAll('.card .en').forEach(en=>{
  ...
  function buildChunks(){
    ...
    for(const s of sentences){
      if(byteLen(s.text)>MAX_REQUEST_BYTES){flush();out.push(s);continue;}
      const candidate=(buf?buf+' ':'')+s.text;
      if(byteLen(candidate)<=MAX_REQUEST_BYTES){
  ...
  function updateProgress(){
    const p=document.getElementById('gcttsProgress');const l=document.getElementById('gcttsProgressLabel');
  ...
  async function playCurrent(){
    ...
    try{
      const blob=await synthesize(c.text);
      const url=URL.createObjectURL(blob);
      if(audio){audio.pause();audio=null;}
      audio=new Audio(url);
      audio.onloadedmetadata=()=>{highlightWord(c.start||0,true);};
      audio.onplay=()=>{stopCursor();cursorLoop();};
      audio.onpause=()=>stopCursor();
      audio.onended=()=>{URL.revokeObjectURL(url);stopCursor();if(!playing)return;highlightWord((c.end||c.start)-1,false);chunkIndex++;if(chunkIndex>=chunks.length){playing=false;status('播放完成');updateProgress();return;}playCurrent();};
  ...
  function playAll(){
    ...
    console.log('Google Cloud TTS v4 chunks:',chunks.map((c,i)=>({i:i+1,bytes:byteLen(c.text),start:c.start,end:c.end,preview:c.text.slice(0,100)})));
    playCurrent();
  }
  function stop(){playing=false;stopCursor();if(audio){audio.pause();audio.currentTime=0;}status('已停止');clearHighlight();}
  ...
  function inject(){
    ...
    console.log('Google Cloud TTS reader patch v4 loaded');
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',inject);else inject();
  window.openGoogleCloudTTSKey=setKey;
  window.googleCloudTTSPlayAll=playAll;
  window.googleCloudTTSSplitDebug=()=>buildChunks().map((c,i)=>({i:i+1,bytes:byteLen(c.text),start:c.start,end:c.end,text:c.text}));
})();
